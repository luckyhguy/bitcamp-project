...
public class ContactController {
	...
	/*
	 * private void doUpdate() { System.out.print("변경할 강의실의 인덱스? "); int index =
	 * Integer.parseInt(this.keyScan.nextLine());
	 * 
	 * Contact oldContact = list.get(index);
	 * 
	 * // 새 학생 정보를 입력 받는다. Contact contact = new Contact();
	 * System.out.printf("종류(예:일반)?", oldContact.name); contact.type =
	 * this.keyScan.nextLine();
	 * 
	 * while (true) { try { System.out.printf("크기(예:30)?", oldContact.type);
	 * contact.size = Integer.parseInt(this.keyScan.nextLine()); break; } catch
	 * (Exception e) { System.out.println("정수 값을 입력하세요."); } }
	 * 
	 * System.out.print("전자칠판(y/n)?"); contact.electronicslate =
	 * (this.keyScan.nextLine().equals("y"))? true : false;
	 * 
	 * System.out.print("칠판(y/n)?"); contact.blackboard =
	 * (this.keyScan.nextLine().equals("y")) ? true : false;
	 * 
	 * System.out.print("소방기구(y/n)?"); contact.firefightingequipment =
	 * (this.keyScan.nextLine().equals("y")) ? true : false;
	 * 
	 * System.out.print("사용중(y/n)?"); contact.working =
	 * (this.keyScan.nextLine().equals("y")) ? true : false;
	 * 
	 * System.out.print("저장하시겠습니까(y/n)?"); if
	 * (!this.keyScan.nextLine().equals("y")) { contact.name = oldContact.name;
	 * list.set(index, contact); changed = true; System.out.println("저장하였습니다."); }
	 * else { System.out.println("변경을 취소하였습니다."); } }
	 */
	private void doAdd() {
		while (true) {
